...
def generate_diff_dict(default: CfgNode, cfg: CfgNode, include_missing=False) -> dict:
    ...
    for k, v in cfg.items():
        ...
        if isinstance(v, CfgNode):
            subdiff = generate_diff_dict(
                default[k], cfg[k], include_missing=include_missing
            )
            if len(subdiff) > 0:
                diff[k] = subdiff
        else:
...
def setup_cfg(
    cfg_file=[], set_cfgs=None, default: CfgNode = None, logdir="log/"
) -> CfgNode:
    """
    ...
    logdir = os.path.join(
        logdir, cfg.dataset, cfg.split, cfg.aux.exp, str(cfg.aux.runid)
    )
