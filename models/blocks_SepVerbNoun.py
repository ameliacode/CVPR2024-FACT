...
class FACT(nn.Module):
    ...
    def _forward_one_video(self, seq, transcript=None):
        ...
        if not self.cfg.FACT.trans:
            ...
            vtranscript = torch.LongTensor([_VIDS[a] for a in transcript]).to(
                transcript.device
            )
            ntranscript = torch.LongTensor([_NIDS[a] for a in transcript]).to(
                transcript.device
            )
...
def init_VIDS_NIDS():
    ...
    if (_VIDS is None) or (_NIDS is None):
        ...
        v2i, i2v = load_action_mapping(
            "./data/epic-kitchens/processed/verb_mapping.txt"
        )
        n2i, i2n = load_action_mapping(
            "./data/epic-kitchens/processed/noun_mapping.txt"
        )
...
class Block(nn.Module):
    ...
    def temporal_downsample(self, frame_feature):
        ...
        seg_feature = self.seg_combine(
            seg_feature
        )  # combine forward and backward features of GRU
        seg_feature, seg_clogit = self.process_feature(
            seg_feature, self.nclass1, self.nclass2
        )
    ...
    def eval(self, transcript=None):
        if not self.cfg.FACT.trans:
            return self._eval(
                self.action_logp, self.a2f_attn, self.frame_logp, self.cfg.FACT.mwt
            )
        else:
...
class InputBlockTDU(Block):
    ...
    def forward(self, frame_feature, action_feature, frame_pos, action_pos):
        ...
        frame_feature, frame_clogit = self.process_feature(
            frame_feature, self.nclass1, self.nclass2
        )
        ...
        # action branch
        center = torch.LongTensor([int((s.start + s.end) / 2) for s in tdu.segs]).to(
            seg_feature.device
        )
        seg_pos = frame_pos[center]
        action_feature = self.action_branch(
            action_feature, seg_feature, pos=seg_pos, query_pos=action_pos
        )
        action_feature, action_clogit = self.process_feature(
            action_feature, self.nclass1 + 1, self.nclass2 + 1
        )
        ...
        self.action_logp = self.combine_verb_noun_to_action(
            action_clogit, action=True, apply_log=True
        )
        self.tdu = tdu
        self.f2a_attn = None
    ...
    def compute_loss(self, criterion: loss.MatchCriterion, match=None):
        # frame loss
        frame_loss = (
            criterion.frame_loss(self.frame_logp.squeeze(1), is_logit=False) / 2
        )
        seg_loss = (
            criterion.frame_loss_tdu(self.seg_logp.squeeze(1), self.tdu, is_logit=False)
            / 2
        )
