...
from utils.utils import easy_reduce, expand_frame_label, parse_label
...
class Checkpoint:
    ...
    def _joint_metrics(self, gt_list, pred_list):
        ...
        for gt, pred in zip(gt_list, pred_list):
            ...
            for s in range(len(overlap)):
                tp1, fp1, fn1 = f_score(
                    pred_segs, gt_segs, overlap[s], bg_class=self.bg_class
                )
                tp[s] += tp1
                fp[s] += fp1
                fn[s] += fn1
    ...
    def compute_metrics(self):
        ...
        for vname, video in self.videos.items():
            video.pred_label = expand_frame_label(
                video.pred, len(video.gt_label)
