...
def already_finished(logdir):
    ...
    if os.path.exists(fulldir) and os.path.exists(
        os.path.join(fulldir, "FINISH_PROOF")
    ):
        return True
    else:
        return False
...
def resume_ckpt(cfg, logdir):
    ...
    if cfg.aux.resume == "" or (not os.path.exists(logdir)):
        ...
        iterations = [
            int(os.path.basename(f)[:-4].split("-")[-1]) for f in network_ckpts
        ]
...
def save_results(
    ckpt: Checkpoint, vnames: list, label_list: list, attrs_saves: list
) -> list:
    videos = []
